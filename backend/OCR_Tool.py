...
# # # Example usage:
# # db_path = 'dyslexiadetect.db'  # Replace with your database path
...
#         id, timestamp, left_x, left_y, right_x, right_y, gaze_direction, category_left, category_right = entry
#         screenshot_filename = f"{timestamp}.jpg"  # Assuming screenshots are saved as '.jpg'
#         screenshot_path = os.path.join(user_folder, screenshot_filename)

#         # Check if screenshot file exists
#         if os.path.isfile(screenshot_path):
#             # Call perform_ocr to extract text from the screenshot
#             ocr_text = perform_ocr(left_x, left_y, right_x, right_y, screenshot_path)
#             user_ocr_data.append([timestamp, ocr_text])
        
#     # Write OCR data to a CSV file
...
def main():
    db_path = 'dyslexiadetect.db'
    data_folder = 'data'
    user_id = input("Enter user ID: ")  # Allow the user to input their ID

    fixation_data = get_fixation_data(user_id, db_path)
    if fixation_data:
        process_screenshots_for_ocr(data_folder, user_id, fixation_data)
    else:
        print("No fixation data available for processing.")
